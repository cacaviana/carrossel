...
def salvar_imagem(pipeline_id: str, slide_index: int, image_b64: str, formato: str = "carrossel") -> str:
    ...
    try:
        ...
        # Forcar dimensoes do formato: resize + crop central se aspect ratio nao bate
        ...
        # Regras: nao cortar, nao distorcer.
        # - Ratio bate (tolerancia 2%): resize pra target (upscale limpo)
        # - Ratio diferente: preserva imagem original (tamanho do Gemini). Melhor
        #   um slide com tamanho "errado" do que um com pedaco cortado ou esticado.
        if abs(src_ratio - target_ratio) <= 0.02 and (img.width != target_w or img.height != target_h):
