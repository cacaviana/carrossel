<script lang="ts">
	...
	const mainItems: NavItem[] = [
		...
		{ href: '/anuncios', label: 'Anuncios', icon: 'M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z' },
		{ href: '/historico', label: 'Historico', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
	];
	...
	const formatItems: NavItem[] = [
		...
		{ href: '/?formato=anuncio', label: 'Anuncio', icon: 'M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z', badge: 'Novo', badgeVariant: 'cyan' },
				...
				{:else}
					<a href={item.href}
						...
						{#if !collapsed}
							<span>{item.label}</span>
							{#if item.badge}
								{#if item.badgeVariant === 'cyan'}
									<span class="ml-auto text-[10px] px-1.5 py-0.5 rounded-full bg-cyan-400/10 text-cyan-600 border border-cyan-400/30 font-mono uppercase">{item.badge}</span>
								{:else}
									<span class="ml-auto text-[10px] px-1.5 py-0.5 rounded-full bg-purple/8 text-purple-soft border border-purple/20">{item.badge}</span>
								{/if}
							{/if}
