<script lang="ts">
	...
	const formatItems: NavItem[] = [
		...
		{ href: '#', label: 'Reels', icon: 'M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z', badge: 'Em breve', disabled: true },
		...
		{ href: '/?formato=funil', label: 'Funil', icon: 'M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12' },
	];
	...
	const legacyItems: NavItem[] = [
		{ href: '/carrossel', label: 'Carrossel', icon: 'M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7', badge: 'legado', badgeVariant: 'amber' },
	];
...
<aside class="flex flex-col h-screen bg-bg-card border-r border-border-default transition-all duration-300
	...
	<nav class="flex-1 overflow-y-auto py-3 px-2 space-y-4 scrollbar-hide">
		...
		<!-- Legado -->
		<div>
			{#if !collapsed}<p class="label-upper px-3 mb-2">Legado</p>{/if}
			{#each legacyItems as item}
				<a href={item.href}
					...
					{#if !collapsed}
						<span>{item.label}</span>
						{#if item.badge}
							<span class="ml-auto text-[10px] px-1.5 py-0.5 rounded-full bg-amber/10 text-amber border border-amber/25 font-mono">{item.badge}</span>
						{/if}
					{/if}
				</a>
			{/each}
